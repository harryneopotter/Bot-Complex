...
const BotCard = React.memo(({ bot, onSelectBot }: { bot: Bot, onSelectBot: (bot: Bot) => void }) => {
...
const HomePage = React.memo(({ setActiveTab }: { setActiveTab: (tab: "home" | "bots") => void, displayBots?: Bot[], onSelectBot?: (bot: Bot) => void }) => (
...
const BotsGallery = React.memo(({ setActiveTab, displayBots, onSelectBot }: { setActiveTab: (tab: "home" | "bots") => void, displayBots: Bot[], onSelectBot: (bot: Bot) => void }) => (
    ...
    <div className="px-8 py-12">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8 px-4">
          {displayBots.map((bot) => (
            <BotCard key={bot.id} bot={bot} onSelectBot={onSelectBot} />
          ))}
        </div>
      </div>
    </div>
...
const App = () => {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'bots'>('home');
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [serverBots, setServerBots] = useState<any[] | null>(null);
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([]);
  const [chatInput, setChatInput] = useState('');
  const [isStreaming, setIsStreaming] = useState(false);
  const [sessionId, setSessionId] = useState('');
  // streamMeta is used by something else potentially? Wait, setStreamMeta is called but streamMeta is never read.
  const [, setStreamMeta] = useState<StreamMeta | null>(null);
  ...
  async function streamChat({ botId, messages }: { botId: number, messages: ChatMessage[] }) {
  ...
  const availableIds = React.useMemo(() => new Set((serverBots || []).map(b => b.id)), [serverBots]);
  const displayBots = React.useMemo(() => availableIds.size ? bots.filter(b => availableIds.has(b.id)) : bots, [availableIds]);
  ...
  return (
    <div className="font-sans">
      {activeTab === 'home' && <HomePage setActiveTab={setActiveTab} displayBots={displayBots} onSelectBot={handleSelectBot} />}
      {activeTab === 'bots' && <BotsGallery setActiveTab={setActiveTab} displayBots={displayBots} onSelectBot={handleSelectBot} />}
      <ChatModal
