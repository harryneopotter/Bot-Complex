...
function upsertSession(sessionId, record) {
    const now = Date.now();
    const value = { history: [], ...record, updatedAt: now };
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        const { sessionId = requestId, botId, messages = [], quality = 'standard', // 'standard' | 'high'
        providerHint, modelHint, temperature, max_tokens, } = req.body || {};
        if (!botId) {
        ...
        res.write(`data: ${JSON.stringify({ code: 'UPSTREAM_ERROR', message: String(lastError?.message || 'All providers failed'), requestId })}\n\n`);
        res.end();
    }
    catch (e) {
        if (!res.headersSent) {
