...
import personasData from './bots/registry.json' with { type: 'json' };
...
function upsertSession(sessionId: string, record: Partial<SessionData>) {
  const now = Date.now();
  const value: SessionData = { history: [], ...record, updatedAt: now };
...
interface ChatRequestBody {
  ...
  providerHint?: 'groq' | 'together' | 'chutes';
  modelHint?: string;
  temperature?: number;
  max_tokens?: number;
}
...
app.post('/api/chat', async (req: Request<{}, {}, ChatRequestBody>, res: Response) => {
  ...
  try {
    ...
    if (!botId) {
      res.status(400).json({ code: 'BAD_REQUEST', message: 'botId is required', requestId });
      return;
    }
    ...
    if (!persona) {
      res.status(404).json({ code: 'BOT_NOT_FOUND', message: `Unknown botId: ${botId}` });
      return;
    }

    const previous = getSession(sessionId)?.history || [];
    const { composed } = composeMessages(persona, previous, messages);
    ...
    const PERSONA_PREFS: Record<number, { standard?: Partial<ChatOptions>, high?: Partial<ChatOptions> }> = {
    ...
    const ladder: Partial<ChatOptions>[] = [...base];

    // Helper to deduplicate provider/model steps
    const uniq = (arr: Partial<ChatOptions>[]) => {
      const seen = new Set();
      return arr.filter((s) => {
