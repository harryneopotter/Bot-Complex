...
import { ChatMessage, ChatOptions } from './types.js';
...
interface RequestChatStreamArgs {
  messages: ChatMessage[];
  options: ChatOptions;
  signal: AbortSignal;
}

export async function requestChatStream({ messages, options, signal }: RequestChatStreamArgs) {
  ...
  return fetch(conf.endpoint, {
    ...
    duplex: 'half',
  } as any); // Cast to any because undici types might mismatch with global fetch or specific options
}
