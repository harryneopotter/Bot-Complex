...
function buildResidencyPremise(persona: Persona): ChatMessage | null {
...
export function composeMessages(persona: Persona, history: ChatMessage[] = [], newMessages: ChatMessage[] = []) {
  const residency = buildResidencyPremise(persona);
  const sys: ChatMessage = { role: 'system', content: persona.system_prompt };
  ...
  const dev: ChatMessage | null = (persona.style_guidelines?.length || persona.constraints?.length)
    ? { role: 'system', content: [
  ...
  const few: ChatMessage[] = (persona.few_shots || []).flatMap(ex => ([
    { role: 'user', content: ex.user } as ChatMessage,
    { role: 'assistant', content: ex.assistant } as ChatMessage,
  ]));
